import { defineVue, boot } from "./node_modules/tauri-kargo-tools/src/vue"
import { ChatMessage, chatOllama, listOllamaModels, OllamaModel } from "./ollama"
import { TauriKargoClient } from "./node_modules/tauri-kargo-tools/src/api";
const client = new TauriKargoClient()
const question =
    "crée-moi un unique personnage de film fantastique en une seule ligne ; réponds en français et donne uniquement la description, sans baratin";
let modelName: string | undefined
class Personnage {
    estSelectionne: boolean
    description!: string
    listePersonnage!: ListePersonnage
    constructor() {
        this.estSelectionne = false
    }
    selectionnerPersonnage() {
        this.listePersonnage.selectionnerPersonnage()

    }
}
defineVue(Personnage, {
    kind: 'flow',
    orientation: "row",
    children: [
        {
            kind: 'input', inputType: 'checkbox', name: "estSelectionne", update: "selectionnerPersonnage"

        },
        { kind: 'label', name: "description" }
    ]
})
class ListePersonnage {

    peutLancerConversation: boolean
    afficherSupprimerPersonnages: boolean
    models: OllamaModel[] = []
    modelSelections: number[] = []
    personnages: Personnage[] = []
    conversation!: Conversation
    titre = "Conversation"
    messages: ChatMessage[] = [
        { role: "system", content: "Tu es concis, tu réponds en une seule ligne, uniquement la description du personnage." },
    ];
    constructor() {
        this.peutLancerConversation = false
        this.afficherSupprimerPersonnages = false;
        (async () => {
            const txt = await client.readFileText("personnages.json")
            const r = JSON.parse(txt)
            const personnages: string[] = r;
            const tmpList: Personnage[] = []
            for (const p of personnages) {
                const tmp = new Personnage()
                tmp.listePersonnage = this
                tmp.description = p
                this.messages.push({ role: "user", content: question });
                this.messages.push({ role: "assistant", content: p });
                tmpList.push(tmp)
            }
            this.personnages = tmpList
            this.models = await listOllamaModels()
        })()
    }
    modelName(model: OllamaModel) {
        return model.name
    }
    selectionnerPersonnage() {
        this.afficherSupprimerPersonnages = this.personnages.some((p) => p.estSelectionne)
        const n = this.personnages.filter((p) => p.estSelectionne).length
        this.peutLancerConversation = (n === 2) && !!modelName
    }
    async ajouterPersonnage() {
        const newPersonnage = new Personnage()
        newPersonnage.description = ""
        newPersonnage.listePersonnage = this
        this.personnages = [...this.personnages, newPersonnage]
        this.messages.push({ role: "user", content: question });

        const answer = await chatOllama(this.messages, modelName!, {
            stream: true,
            onToken: (t) => { newPersonnage.description = (newPersonnage.description ?? "") + t; },
        });

        const descriptionPerso = answer.trim();
        newPersonnage.description = descriptionPerso; // version finale propre

        this.messages.push({ role: "assistant", content: descriptionPerso });
        const personnages = this.personnages.map((p) => p.description)
        client.writeFileText("personnages.json", JSON.stringify(personnages, null, 2))



    }
    async supprimerPersonnages() {
        this.personnages = this.personnages.filter((p) => !p.estSelectionne)
        this.selectionnerPersonnage()
        const personnages = this.personnages.map((p) => p.description)
        client.writeFileText("personnages.json", JSON.stringify(personnages, null, 2))


    }
    lancerConversation(): Conversation {
        const r = new Conversation()
        const l = this.personnages.filter((p) => p.estSelectionne)
        r.init(l[0].description, l[1].description)
        return r;


    }
    selectModel() {
        if (this.modelSelections.length > 0) {
            modelName = this.models[this.modelSelections[0]].name
            const n = this.personnages.filter((p) => p.estSelectionne).length
            this.peutLancerConversation = (n === 2) && !!modelName
        }

    }

}

defineVue(ListePersonnage, (vue) => {
    vue.flow({
        orientation: "column", gap: 15, style: { margin: "20px" },
        align: "center"
    }, () => {
        vue.flow({
            orientation: "row",
            width: '100%',
            gap: 15,
        }, () => {

            vue.staticButton({
                action: "ajouterPersonnage",
                label: "Ajouter",
                width: '30%',
                enable:"peutLancerConversation"
            })
            
            vue.staticButton({

                action: "supprimerPersonnages",
                label: "Supprimer",
                enable: "afficherSupprimerPersonnages",

                width: '30%'
            })
            vue.select({ list: "models", displayMethod: "modelName", selection: "modelSelections", update: "selectModel", mode: "dropdown" })

        })
    })
    vue.listOfVue({ list: "personnages", gap: 10, wrap: false, align: "center" })
    vue.bootVue({ factory: "lancerConversation", label: "titre", enable: "peutLancerConversation", width: "100%" })
})

class Echange {
    personnageA!: string
    afficherPersonnageA: boolean
    afficherPersonnageB: boolean
    personnageB!: string

    constructor() {
        this.personnageA = ""
        this.afficherPersonnageA = false;
        this.personnageB = ""
        this.afficherPersonnageB = false

    }
}
defineVue(Echange, (vue) => {
    vue.flow({
        orientation: "column",
        width: "100%",
        gap: 10,

    }, () => {
        vue.flow({
            orientation: "row",
            width: "100%",
            gap: 10,

        }, () => {
            vue.label("personnageA", {
                width: "40%",
                useVisibility: true,
                visible: "afficherPersonnageA"
            });
            vue.space("60%", "")
        })
        vue.flow({
            orientation: "row",
            width: "100%",
            gap: 10
        }, () => {
            vue.space("60%", "")
            vue.label("personnageB", {
                width: "40%",
                useVisibility: true,
                visible: "afficherPersonnageB"
            });

        })
    })
})

class Conversation {
    messagesA: ChatMessage[] = []
    messagesB: ChatMessage[] = []
    echanges: Echange[] = []
    personnageA!: string
    personnageB!: string
    interval: number | undefined
    titreBouton = "Demarer"
    running = false
    constructor() {

    }
    lancerListePersonnage() {
        this.arreter()
        return new ListePersonnage()
    }
    init(personnageA: string, personnageB: string) {
        this.personnageA = personnageA
        this.personnageB = personnageB
        this.messagesA = [
            { role: "system", content: `tu est ${personnageA} et tu parle à  ${personnageB} , tu répond en une ligne` },
            { role: 'user', content: 'Bonjour' }
        ];
        this.messagesB = [
            { role: "system", content: `tu est ${personnageB} et tu parle à  ${personnageA}, tu répond en une ligne` }
        ];
    }
    async faireDialogue() {
        if (this.running) {
            return
        }
        this.running = true
        const echange = new Echange()

        this.echanges = [...this.echanges, echange]
        let r = await chatOllama(this.messagesA, modelName!, {
            stream: true,
            onToken: (t) => {
                echange.afficherPersonnageA = true
                echange.personnageA = echange.personnageA + t
            },
        });

        this.messagesA.push({ role: 'assistant', content: r })

        this.messagesB.push({ role: 'user', content: r })
        let r2 = await chatOllama(this.messagesB, modelName!, {
            stream: true,
            onToken: (t) => {
                echange.afficherPersonnageB = true;
                echange.personnageB = echange.personnageB + t
            },
        });

        this.messagesB.push({ role: 'assistant', content: r2 })
        this.messagesA.push({ role: "user", content: r2 })

        echange.personnageA = r
        echange.personnageB = r2

        this.running = false
    }
    demarer() {
        if (this.interval !== undefined) {
            return;
        }

        this.interval = setInterval(async () => {
            await this.faireDialogue()
        })
    }
    arreter() {
        if (this.interval === undefined) {
            return;
        }
        clearInterval(this.interval)
        this.interval = undefined
    }
    faireAction() {
        if (this.interval === undefined) {
            this.titreBouton = "Arreter"
            this.demarer()
            return
        }
        this.titreBouton = "Demarer"
        this.arreter()
    }
    titreListePersonnage = "Liste personnages"
    

}
defineVue(Conversation, (vue) => {
    vue.flow({
        orientation: "column",
        gap: 30
    }, () => {
        vue.flow({
            orientation: "row",
            width: "100%",
            gap: 10
        }, () => {
            vue.label("personnageA", { width: "40%" })
            vue.button({ label: "titreBouton", action: "faireAction", width: "10%" })
            vue.bootVue({  factory:"lancerListePersonnage" , label:"titreListePersonnage",width:"10%"})
            vue.label("personnageB", { width: "40%" })
        })
        vue.listOfVue({
            list: "echanges",
            gap: 15,
            style: { overflow: "auto" },
            wrap: false,
            height: 500
        })
    })
})

boot(new ListePersonnage())